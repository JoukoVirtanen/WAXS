...
void FindNearestHydrationRadius(CubeStruct &Cube, vector<AtomStruct> &Atoms, int natom, Real LargestRadius)
{
        ...
        Real d=0.1;
        if (abs(Cube.x-18.63)<d && abs(Cube.y+3.001)<d && abs(Cube.z-5.869)<d)
        {
                BoolPrint=true;
        }
                        ...
                        if (dy<Maxdist && dy>-Maxdist)
                        {
                                ...
                                if (dz<Maxdist && dz>-Maxdist)
                                {
                                        ...
                                        if (BoolPrint)
                                        {
                                                ...
                                                cout <<"Cube.SecondMinDist= "<<Cube.SecondMinDist<<" Cube.SecondNearest= "<<Cube.SecondNearest<<endl;
                                                cout <<endl;
                                        }
                                }
                        }
...
void VerySafeMakeGrid(Real xmin, Real ymin, Real zmin, Real xmax, Real ymax, Real zmax, vector<AtomStruct> &Atoms, Real maxhs, Real CubeSize, vector<CubeStruct> &Cubes, string PdbFile, Matrix &AtomTypesDensityMatrix, Matrix &ElementsDensityMatrix, bool PhobicPhilic, Real BulkDensity, ParamStruct &params)
{
        bool MakeGridSafely=false;
        ...
        InitializeCube(Cube);
        LargestRadius=GetLargestRadius(Atoms);
        for (int i=0;i<Atoms.size();i++) PrintAtomToFile(PdbFile, Atoms[i]);
        for (xgrid=xmin;xgrid<=xmax;xgrid+=CubeSize)
        {
                cout <<"xgrid= "<<xgrid<<" xmax= "<<xmax<<endl;
                bool BoolPrint=false;
...
void AssignCubeDensities(vector<CubeStruct> &Cubes, vector<AtomStruct> &Atoms, Matrix &AtomTypesDensityMatrix, Matrix &ElementsDensityMatrix, Real BulkDensity, bool UniformHydrationShellDensity, Real HydrationShellDensity, ParamStruct &params, Real RecBin)
{
        ...
        for (n=0;n<GridPoints;n++)
        {
                ...
                if (Cubes[n].LocatedIn==InHydrationShell)
                {
                        ...
                        start=clock();
                        if (n>3100) cout <<"About to enter GetDensity"<<endl;
                        if (UniformHydrationShellDensity) density=HydrationShellDensity;
                        else SetDensity=GetDensity(density, Cubes[n], AtomTypesDensityMatrix, ElementsDensityMatrix, BulkDensity, params, RecBin, Atoms);
                        if (n>3100) cout <<"Left GetDensity n= "<<n<<endl;
                        if (!SetDensity)
                        {
                                PrintCubeInfo(Cubes[n]);
                        ...
                        if (n>3100) cout <<"density= "<<density<<endl;
                ...
                if (Cubes[n].LocatedIn==InSolution)
                {
                        Cubes[n].density=BulkDensity;
                }
        }
...
void BuildHydrationShell(vector<CubeStruct> &Cubes, vector<AtomStruct> &Atoms, Matrix &AtomTypesDensityMatrix, Matrix &ElementsDensityMatrix, Real BulkDensity, bool UniformHydrationShell, Real hsdensity, ParamStruct params)
{
        ...
        AssignCubeDensities(Cubes, Atoms, AtomTypesDensityMatrix, ElementsDensityMatrix, BulkDensity, UniformHydrationShell, hsdensity, params, params.RecBin);
        seconds2=time(NULL);
