#ifndef _WeightedSumCubes_included_
#define _WeightedSumCubes_included_
...
void WeightedSumCubes(vector<CubeStruct> &Cubes1, vector<CubeStruct> &Cubes2, vector<CubeStruct> &AverageCubes, Real NumStructures1, Real NumStructures2, Real bin)
{
	...
	for (m=0;m<CubeNum;m++)
	{
		...
		if (Exists2[xgrid][ygrid][zgrid])
		{
			...
			Cube.density=(Cubes1[m].density*NumStructures1+CubeDensityXYZ*NumStructures2);
		...
		else Cube.density=Cubes1[m].density*NumStructures1;
	...
	for (m=0;m<CubeNum2;m++)
	{
		...
		if (!Exists1[xgrid][ygrid][zgrid])
		{
			...
			Cube.y=Cubes2[m].y;
			Cube.z=Cubes2[m].z;
			Cube.density=Cubes2[m].density*NumStructures2;
