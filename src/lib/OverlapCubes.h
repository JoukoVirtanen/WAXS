...
Real OverlapCubes(vector<CubeStruct> &Cubes1, vector<CubeStruct> &Cubes2, vector<AtomStruct> &OverlapAtoms, vector<AtomStruct> &BlockedAtoms, bool PrintDifferencePdb)
{
        bool verbose=false;
	...
	for (m=0;m<CubeNum;m++)
	{
		...
		if (true)
		{
			...
			if (Cubes1[m].density!=0 && CubeDensityXYZ==0)
			{
				SolventMisidentifiedAsProtein+=1.0;
			        ...
			        //Atom.weight=Cubes1[m].density;
                                ...
                                SafePushBack(BlockedAtoms, Atom, "BlockedAtoms");
			}
			...
			if (Cubes1[m].density!=0 && CubeDensityXYZ!=0)
			{
			        ...
			        //Atom.weight=Cubes1[m].density;
                                ConvertCubeToAtom(Cubes1[m], Atom);
                                SafePushBack(OverlapAtoms, Atom, "OverlapAtoms");
                        }
...
Real OverlapCubes(vector<AtomStruct> &Atoms1, vector<AtomStruct> &Atoms2, vector<AtomStruct> &OverlapAtoms, vector<AtomStruct> &BlockedAtoms, bool PrintDifferencePdb)
{
        ...
        OverlapCubes(Cubes1, Cubes2, OverlapAtoms, BlockedAtoms, PrintDifferencePdb);
