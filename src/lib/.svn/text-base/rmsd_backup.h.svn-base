...
Real RMSDNoAlign(vector<AtomStruct> Atoms1, vector<AtomStruct> &Atoms2)
{
	...
	for (i=0;i<natom;i++)
	{
		if (Atoms1[i].AtomName=="CA")
		{
			dist=AtomDistance(Atoms1[i], Atoms2[i]);
			rmsd+=dist*dist;
			NumAve+=1.0;
		}
	}
...
Real RMSD(vector<AtomStruct> Atoms1, vector<AtomStruct> &Atoms2)
{
	...
	CalcCenter(Atoms2, XOrigin, YOrigin, ZOrigin);
	//cout <<"XOrigin= "<<XOrigin<<" YOrigin= "<<YOrigin<<" ZOrigin= "<<ZOrigin<<endl;
	CalcCenter(Atoms1, XOrigin, YOrigin, ZOrigin);
	//cout <<"XOrigin= "<<XOrigin<<" YOrigin= "<<YOrigin<<" ZOrigin= "<<ZOrigin<<endl;
	center(Atoms1);
	center(Atoms2);
	rmsd_old=GridSearch(Atoms1, Atoms2, theta, phi, psi);
	//rmsd_old=RMSDNoAlign(Atoms1, Atoms2);
	while (true)
	{
		gradient=CalcGradient(Atoms1, Atoms2, derivative, theta, phi, psi);
		if (gradient<GradientCriterion) break;
		rmsd=MinimizeAlongVector(Atoms1, Atoms2, derivative, theta, phi, psi);
		dRMSD=rmsd-rmsd_old;
