...
void FindNearestVDWSurface(CubeStruct &Cube, vector<AtomStruct> &Atoms, int natom, Real LargestRadius)
{
        ...
        {
                if (IsProtein(Atoms[m]) || isNA(Atoms[m]))
                {
                        dx=Cube.x-Atoms[m].x;
        ...
        if (abs(Cube.x+20.75)<0.1 && abs(Cube.y-1.250)<0.1 && abs(Cube.z+3.75)<0.1)
        {
                cout <<"In FindNearestVDWSurface"<<endl;
                PrintCubeInfo(Cube);
                PrintAtomInfo(Atoms[Cube.nearest]);
                cout <<"vdw= "<<Atoms[Cube.nearest].vdw<<endl;
        }
}
...
void FindNearestHydrationRadius(CubeStruct &Cube, vector<AtomStruct> &Atoms, int natom, Real LargestRadius)
{
        ...
        Real d=0.1;
        if (abs(Cube.x-7.0)<d && abs(Cube.y-13.0)<d && abs(Cube.z-1000.6)<d)
        {
                BoolPrint=true;
...
void FindNearestHydrationAndVdwRadii(CubeStruct &Cube, vector<AtomStruct> &Atoms, int natom, Real LargestRadius, Real maxhs, int &previousSecondNearest)
{
        bool BoolPrint=false;
        int TotalParticles=Atoms.size();
        Real Maxdist, SecondMinDist;
        Real r, dx, dy, dz;
        Real probeRadius=1.4;
        Real d=0.1, dist, minDistToVdw, distToVdw;
        minDistToVdw=1000.0;
        Cube.MinDist=1000.0;
        SecondMinDist=1000.0;

        Cube.SecondNearest=previousSecondNearest;
        dx=Cube.x-Atoms[previousSecondNearest].x;
        dy=Cube.y-Atoms[previousSecondNearest].y;
        dz=Cube.z-Atoms[previousSecondNearest].z;
        r=sqrt(dx*dx+dy*dy+dz*dz)-Atoms[previousSecondNearest].HydrationRadius;
        Maxdist=r+LargestRadius+1.0;
        if (abs(Cube.x-11.4)<d && abs(Cube.y-39)<d && abs(Cube.z-45.4)<d)
        {
                BoolPrint=true;
                cout <<"In FindNearestHydrationAndVdwRadius"<<endl;
        }
        for (int n=0;n<TotalParticles;n++)
        {
                if (Atoms[n].solute)
                        ...
                        if (dx<Maxdist && dx>-Maxdist)
                        {
                                ...
                                if (dy<Maxdist && dy>-Maxdist)
                                {
                                        ...
                                        if (dz<Maxdist && dz>-Maxdist)
                                        {
                                                ...
                                                r=dist-Atoms[n].HydrationRadius;
                                                distToVdw=dist-Atoms[n].vdw;
                                                if (distToVdw<minDistToVdw) 
                                                {
                                                        minDistToVdw=distToVdw;
                                                }
                                                if (r<Cube.MinDist)
                                                {
                                                        SecondMinDist=Cube.MinDist;
        ...
        Cube.MinDist+=Atoms[Cube.nearest].HydrationRadius;
        previousSecondNearest=Cube.SecondNearest;
        if (minDistToVdw>probeRadius && dist<maxhs) 
        {
                Cube.LocatedIn=InHydrationShell;
        }
}
...
void DetermineNearestForAllCubes(vector<CubeStruct> &Cubes, vector<AtomStruct> &Atoms, int natom, Real maxhs)
{
        ...
        for (int m=0;m<GridPoints;m++)
        {
                try
                {
                        ...
                        if (Cubes[m].LocatedIn==InHydrationShell || Cubes[m].LocatedIn==InSolution)  //Take out the InSolution later
                        {
        ...
        for (int n=0;n<GridPoints;n++)
        {
                ...
                if (Cubes[n].LocatedIn==InSolution && Cubes[n].MinDist<maxhs+0.5)    //Fix
                {
                        Cubes[n].LocatedIn=InHydrationShell;
                }
                if (Cubes[n].LocatedIn==InHydrationShell)
                {
                        if (Cubes[n].MinDist>maxhs+0.5) Cubes[n].LocatedIn=InSolution; //Fix Cubes[n].MinDist>maxhs
                }
...
void location4(int natom, Real xmin, Real xmax, Real ymin, Real ymax, Real zmin, Real zmax, vector<CubeStruct> &Cubes, vector<AtomStruct> Atoms, Real maxhs)
{
        ...
        for (int n=0;n<GridPoints;n++)
        {
                if (abs(Cubes[n].x+20.75)<0.1 && abs(Cubes[n].y-1.250)<0.1 && abs(Cubes[n].z+3.75)<0.1)
                {
                        cout <<"Cube exists"<<endl;
                }
        }
                                                ...
                                                if (Cubes[PairList2[n]].LocatedIn==InProtein)
                                                {
                                                        ...
                                                        if (dz<ProbeRadius && dz>-ProbeRadius)
                                                        {
                                                                ...
                                                                if ( (dx*dx+dy*dy+dz*dz)<ProbeRadius2)
                                                                {
                                                                        ...
                                                                        if (abs(Cubes[PairList2[n]].x+20.75)<0.1 && abs(Cubes[PairList2[n]].y-1.250)<0.1 && abs(Cubes[PairList2[n]].z+3.75)<0.1)
                                                                        {
                                                                                cout <<"Found the missing cube"<<endl;
                                                                                PrintCubeInfo(Cubes[PairList2[n]]);
                                                                        }
...
Real calcDistances(Real x, Real y, Real z, vector<int> &nearestList, vector<Real> &dist, vector<Real> &minPossibleDist, vector<Real> &maxPossibleDist, int numCalc, vector<AtomStruct> &Atoms, int &nearest, bool boolPrint)
{
        int natom=dist.size();
        Real minDist=dist[0];
        Real d=0.05;
        ...
        for (int i=0;i<numCalc;i++)
        {
                ...
                //cout <<"nearestList["<<i<<"]= "<<nearestList[i]<<endl;
                //PrintAtomInfo(Atoms[nearestList[i]]);
                dist[nearestList[i]]=AtomDistance(x, y, z, Atoms[nearestList[i]]);
                //cout <<"dist["<<i<<"]= "<<dist[i]<<endl;
                dist[nearestList[i]]-=Atoms[nearestList[i]].vdw;
                //cout <<"dist["<<i<<"]= "<<dist[i]<<endl;
                //cout <<endl;
                if (dist[nearestList[i]]<minDist) 
                {
                        minDist=dist[nearestList[i]];
                        nearest=nearestList[i];
                }
        }
        if (boolPrint) cout <<"x= "<<x<<" y= "<<y<<" z= "<<z<<endl;
        for (int i=0;i<natom;i++)
        {
                if (boolPrint)
                {
                        cout <<"dist["<<i<<"]= "<<dist[i]<<" realDist= "<<AtomDistance(x, y, z, Atoms[i])-Atoms[i].vdw<<" minPossibleDist= "<<minPossibleDist[i]<<" maxPossibleDist= "<<maxPossibleDist[i]<<endl;
                }
        }
        return minDist;
}

int updateNearestList(Real minOfMax, vector<Real> &dist, vector<int> &nearestList, vector<Real> &minPossibleDist)
{
        int natom=minPossibleDist.size();
        int numCalc=0;
        for (int i=0;i<natom;i++)
        {
                if (minOfMax>minPossibleDist[i])
                {
                        nearestList[numCalc]=i;
                        numCalc++;
                }
        }
        return numCalc;
...
void updatePossibleDist(vector<Real> &dist, vector<int> &nearestList, int &numCalc, vector<Real> &minPossibleDist, vector<Real> &maxPossibleDist, Real inc)
{
        int natom=maxPossibleDist.size();
        Real minOfMax;
        for (int i=0;i<numCalc;i++)
        {
                maxPossibleDist[nearestList[i]]=dist[nearestList[i]];
                minPossibleDist[nearestList[i]]=dist[nearestList[i]];
        }
        for (int i=0;i<natom;i++)
        {
                maxPossibleDist[i]+=inc;
                minPossibleDist[i]-=inc;
        }
        minOfMax=maxPossibleDist[0];
        for (int i=0;i<natom;i++)
        {
                if (maxPossibleDist[i]<minOfMax)
                {
                        minOfMax=maxPossibleDist[i];
                }
        }
        numCalc=updateNearestList(minOfMax, dist, nearestList, minPossibleDist);
}

bool cornerSolvation(Real x, Real y, Real z, Real probeRadius, CubeStruct Cubes, Real cubeSize)
{
        Real r2, probeRadius2=probeRadius*probeRadius;
        Real dx, dy, dz;
        for (Real i=-1.0;i<=1.1;i+=2.0)
        {
                for (Real j=-1.0;j<=1.1;j+=2.0)
                {
                        for (Real k=-1.0;k<=1.1;k+=2.0)
                        {
                                dx=x-Cubes.x+i*cubeSize*0.5;
                                dy=y-Cubes.y+j*cubeSize*0.5;
                                dz=z-Cubes.z+k*cubeSize*0.5;
                                r2=dx*dx+dy*dy+dz*dz;
                                if (r2<probeRadius2)
                                {
                                        return true;
...
void setCubesWithinProbeRadiusToHydrationShell(Real x, Real y, Real z, Real probeRadius, lattice &Cubes, Real cubeSize, bool checkCorners)
{
        Real probeRadius2=probeRadius*probeRadius;
        Real xmin, ymin, zmin;
        Real xmax, ymax, zmax;
        Real dx, dy, dz;
        Real r2;
        ...
        GetBins(x-probeRadius, y-probeRadius, z-probeRadius, xmin, ymin, zmin, xstart, ystart, zstart, cubeSize);
        GetBins(x+probeRadius, y+probeRadius, z+probeRadius, xmin, ymin, zmin, xend, yend, zend, cubeSize);
        Get3DVectorSize(Cubes, MaxXBin, MaxYBin, MaxZBin, "Cubes");
        ...
        //cout <<"xstart= "<<xstart<<" ystart= "<<ystart<<" zstart= "<<zstart<<" xend= "<<xend<<" yend= "<<yend<<" zend= "<<zend<<" x= "<<x<<"y= "<<y<<" z= "<<z<<endl;
        ...
        for (int i=xstart;i<=xend;i++)
        {
                for (int j=ystart;j<=yend;j++)
                {
                        for (int k=zstart;k<=zend;k++)
                        {
                                if (checkCorners)
                                {
                                        accessible=cornerSolvation(x, y, z, probeRadius, Cubes[i][j][k], cubeSize);
                                        if (accessible) Cubes[i][j][k].LocatedIn=InHydrationShell;
                                }
                                else
                                {
                                        dx=x-Cubes[i][j][k].x;
                                        ...
                                        if (r2<probeRadius2)
                                        {
                                                Cubes[i][j][k].LocatedIn=InHydrationShell;
                                        }
                                }
                        }
                }
...
void initNearestList(vector<int> &nearestList, int natom)
{
        if (int(nearestList.size())!=natom)
        {
                SafeAlloc(nearestList, natom, "nearestList");
        }
        for (int i=0;i<natom;i++)
        {
                nearestList[i]=i;
...
void location(lattice &Cubes, vector<AtomStruct> &Atoms, Real maxhs, ParamStruct &params)
{
        bool BoolPrint=false;
        int MaxXBin, MaxYBin, MaxZBin;
        int numCalc, natom=Atoms.size();
        int nearest;
        vector<int> nearestList;
        Real nearestDist, lastNearestDist, probeRadius=1.4;
        Real xmin, ymin, zmin;
        Real xmax, ymax, zmax;
        Real inc=0.5;
        vector<Real> dist, maxPossibleDist, minPossibleDist;

        SafeAlloc(dist, natom, "dist");
        SafeAlloc(maxPossibleDist, natom, "maxPossibleDist");
        SafeAlloc(minPossibleDist, natom, "minPossibleDist");
        MinMax(xmin, ymin, zmin, xmax, ymax, zmax, Cubes);
        Get3DVectorSize(Cubes, MaxXBin, MaxYBin, MaxZBin, "Cubes");
        cout <<"In location"<<endl;
        cout <<"xmin= "<<xmin<<" ymin= "<<ymin<<" zmin= "<<zmin<<endl;
        cout <<"xmax= "<<xmax<<" ymax= "<<ymax<<" zmax= "<<zmax<<endl;
        for (Real xgrid=xmin;xgrid<xmax;xgrid+=inc)
        {
                for (Real ygrid=ymin;ygrid<ymax;ygrid+=inc)
                {
                        initNearestList(nearestList, natom);
                        lastNearestDist=AtomDistance(xgrid, ygrid, zmin, Atoms[0]);
                        nearestDist=lastNearestDist;
                        numCalc=natom;
                        for (Real zgrid=zmin;zgrid<zmax;zgrid+=inc)
                        {
                                Real d=0.5;
                                //if (abs(xgrid-25.653)<d && abs(ygrid-28.828)<d && abs(zgrid-8.560)<d) BoolPrint=true;
                                //else BoolPrint=false;
                                nearestDist=calcDistances(xgrid, ygrid, zgrid, nearestList, dist, minPossibleDist, maxPossibleDist, numCalc, Atoms, nearest, BoolPrint);
                                if (zgrid==zmin) maxPossibleDist=minPossibleDist=dist;
                                //if (nearestDist>probeRadius && nearestDist<maxhs)
                                if (nearestDist>probeRadius && nearestDist+Atoms[nearest].vdw<8.05)
                                {
                                        setCubesWithinProbeRadiusToHydrationShell(xgrid, ygrid, zgrid, probeRadius, Cubes, params.CubeSize, false);
                                }
                                if (BoolPrint) cout <<"nearestDist= "<<nearestDist<<endl;
                                updatePossibleDist(dist, nearestList, numCalc, minPossibleDist, maxPossibleDist, inc);
                        }
                }
        }
}

Real calcDistances(CubeStruct &Cube, vector<int> &nearestList, vector<Real> &dist, vector<Real> &minPossibleDist, vector<Real> &maxPossibleDist, int numCalc, vector<AtomStruct> &Atoms)
{
        int natom=dist.size();
        Real secondMinDist;
        Cube.nearest=0;
        Cube.SecondNearest=0;
        Cube.MinDist=AtomDistance(Cube.x, Cube.y, Cube.z, Atoms[nearestList[0]]);
        secondMinDist=Cube.MinDist;
        for (int i=0;i<numCalc;i++)
        {
                PrintCubeInfo(Cube);
                //cout <<"numCalc= "<<numCalc<<endl;
                //cout <<"nearestList["<<i<<"]= "<<nearestList[i]<<endl;
                //PrintAtomInfo(Atoms[nearestList[i]]);
                dist[nearestList[i]]=AtomDistance(Cube.x, Cube.y, Cube.z, Atoms[nearestList[i]]);
                dist[nearestList[i]]-=Atoms[nearestList[i]].HydrationRadius;
                if (dist[nearestList[i]]<Cube.MinDist)
                {
                        Cube.SecondNearest=Cube.nearest;
                        Cube.nearest=nearestList[i];
                        secondMinDist=Cube.MinDist;
                        Cube.MinDist=dist[nearestList[i]];
                }
                if (dist[nearestList[i]]<secondMinDist && dist[nearestList[i]]>Cube.MinDist)
                {
                        secondMinDist=dist[nearestList[i]];
                        Cube.SecondNearest=nearestList[i];
                }
        }
        for (int i=0;i<natom;i++)
        {
                //cout <<"dist["<<i<<"]= "<<dist[i]<<" realDist= "<<AtomDistance(Cube.x, Cube.y, Cube.z, Atoms[i])-Atoms[i].HydrationRadius<<" minPossibleDist= "<<minPossibleDist[i]<<" maxPossibleDist= "<<maxPossibleDist[i]<<endl;
        }
        Cube.MinDist+=Atoms[Cube.nearest].HydrationRadius;
        return Cube.MinDist-Atoms[Cube.nearest].HydrationRadius;
}

void determineNearestForAllCubes(lattice &Cubes, vector<AtomStruct> &Atoms, Real maxhs, Real cubeSize)
{
        int MaxXBin, MaxYBin, MaxZBin;
        int numCalc, natom=Atoms.size();
        vector<int> nearestList;
        ...
        for (int xbin=0;xbin<MaxXBin;xbin++)
        {
                ...
                {
                        initNearestList(nearestList, natom);
                        lastNearestDist=AtomDistance(Cubes[xbin][ybin][0].x, Cubes[xbin][ybin][0].y, Cubes[xbin][ybin][0].z, Atoms[0]);
                        numCalc=natom;
                        for (int zbin=0;zbin<MaxZBin;zbin++)
                        {
                                nearestDist=calcDistances(Cubes[xbin][ybin][zbin], nearestList, dist, minPossibleDist, maxPossibleDist, numCalc, Atoms);
                                if (zbin==0) maxPossibleDist=minPossibleDist=dist;
                                updatePossibleDist(dist, nearestList, numCalc, minPossibleDist, maxPossibleDist, cubeSize);
                                if (Cubes[xbin][ybin][zbin].MinDist>8.05) Cubes[xbin][ybin][zbin].LocatedIn=InSolution;
                        }
                }
        }
}

void locationAndDistance(lattice &Cubes, vector<AtomStruct> &Atoms, Real maxhs, ParamStruct &params)
{
        location(Cubes, Atoms, maxhs, params);
        determineNearestForAllCubes(Cubes, Atoms, maxhs, params.CubeSize);
}
...
int makePairList(vector<AtomStruct> &Atoms, AtomStruct &Atom, Real probeRadius, int *PairList)
{
        int PairListNum=0;
        int natom=Atoms.size();
        Real dist, cutOff2;
        for (int i=0;i<natom;i++)
        {
                dist=AtomSquareDistance(Atoms[i], Atom);
                cutOff2=(Atom.vdw+Atoms[i].vdw+probeRadius)*(Atom.vdw+Atoms[i].vdw+probeRadius);
                if (dist<cutOff2)
                {
                        PairList[PairListNum]=i;
                        PairListNum++;
                }
        }
        return PairListNum;
...
bool fastLocation(lattice &Cubes, vector<AtomStruct> &Atoms, Real cubeSize, bool predictCorners)
{
        bool SphereOverlap;
        int natom=Atoms.size();
        int numThetaSlices=10, numPhiSlices=10;
        int nvector, nearest;
        int PairListNum;
        int *PairList;
        Real probeRadius=1.4, d=0.0001;
        Real CutOff=1000.0, CutOff2;
        Real xgrid, ygrid, zgrid;
        VectorStruct vTemp;
        vector<VectorStruct> v;
        cout <<"In fastLocation"<<endl;
        CutOff2=CutOff*CutOff;
        SafeArrayAlloc(PairList, natom);
        MakePointsOnASphere(numThetaSlices, numPhiSlices, v);
        vTemp.x=0;
        vTemp.y=0;
        vTemp.z=1.0;
        SafePushBack(v, vTemp, "fastLocation");
        vTemp.x=0;
        vTemp.y=0;
        vTemp.z=-1.0;
        SafePushBack(v, vTemp, "fastLocation");
        ...
        for (int i=0;i<natom;i++)
        {
                PairListNum=makePairList(Atoms, Atoms[i], probeRadius, PairList);
                for (int j=0;j<nvector;j++)
                {
                        xgrid=Atoms[i].x+v[j].x*(Atoms[i].vdw+probeRadius+d);
                        ygrid=Atoms[i].y+v[j].y*(Atoms[i].vdw+probeRadius+d);
                        zgrid=Atoms[i].z+v[j].z*(Atoms[i].vdw+probeRadius+d);
                        SphereOverlap=IsOverlapPairlist(Atoms, PairList, PairListNum, CutOff, CutOff2, probeRadius, xgrid, ygrid, zgrid);
                        if (!SphereOverlap)
                        {
                                setCubesWithinProbeRadiusToHydrationShell(xgrid, ygrid, zgrid, probeRadius, Cubes, cubeSize, predictCorners);
                        }
                        //PrintPdbLine("/home2/jouko/Density/TempPdb/Sphere.pdb", true, 1, "O", "HOH", 1, v[j].x*50.0, v[j].y*50.0, v[j].z*50.0, 1.0, 1.0);
                }
                //exit(EXIT_FAILURE);
        }
}

bool fastLocationNoWaters(lattice &Cubes, vector<AtomStruct> Atoms, Real cubeSize, bool predictCorners)
{
        RemoveWaters(Atoms);
        fastLocation(Cubes, Atoms, cubeSize, predictCorners);
}

bool gridLocation(lattice &Cubes, vector<AtomStruct> &Atoms, Real cubeSize, Real maxhs)
{
        bool SphereOverlap;
        int natom=Atoms.size();
        int numThetaSlices=10, numPhiSlices=10;
        ...
        Real CutOff=1000.0, CutOff2;
        Real x, y, z;
        Real LargestRadius, distToSurface;
        CutOff2=CutOff*CutOff;
        Get3DVectorSize(Cubes, MaxXBin, MaxYBin, MaxZBin, "Cubes");
        ...
        for (int xbin=0;xbin<MaxXBin;xbin++)
        {
                for (int ybin=0;ybin<MaxYBin;ybin++)
                {
                        for (int zbin=0;zbin<MaxZBin;zbin++)
                        {
                                FindNearestVDWSurface(Cubes[xbin][ybin][zbin], Atoms, natom, LargestRadius);
                                distToSurface=Cubes[xbin][ybin][zbin].MinDist-Atoms[Cubes[xbin][ybin][zbin].nearest].vdw;
                                if (distToSurface>probeRadius && distToSurface<maxhs)
                                {
                                        Cubes[xbin][ybin][zbin].LocatedIn=InHydrationShell;
                                }
                                if (distToSurface>maxhs)
                                {
                                        Cubes[xbin][ybin][zbin].LocatedIn=InSolution;
                                }
                        }
                }
        }        
...
bool IsOverlap(vector<AtomStruct> &Atoms, Real probeRadius, Real x, Real y, Real z)
{
        int natom=Atoms.size();
        Real cutOff, cutOff2, dist2;
        Real dx, dy, dz;
        for (int i=0;i<natom;i++)
        {
                dx=Atoms[i].x-x;
                ...
                cutOff=Atoms[i].vdw+probeRadius;
                cutOff2=cutOff*cutOff;
                if (dist2<cutOff2) return true;
        }
        return false;
}


bool gridLocation2(lattice &Cubes, vector<AtomStruct> &Atoms, Real cubeSize, Real maxhs)
{
        bool SphereOverlap;
        int natom=Atoms.size();
        int numThetaSlices=10, numPhiSlices=10;
        ...
        for (int xbin=0;xbin<MaxXBin;xbin++)
        {
                for (int ybin=0;ybin<MaxYBin;ybin++)
                {
                        for (int zbin=0;zbin<MaxZBin;zbin++)
                        {
                                ...
                                SphereOverlap=IsOverlap(Atoms, probeRadius, x, y, z);
                                if (!SphereOverlap)
                                {
                                        Cubes[xbin][ybin][zbin].LocatedIn=InHydrationShell;
                                }
...
bool gridLocationNoWaters(lattice &Cubes, vector<AtomStruct> Atoms, Real cubeSize, Real maxhs)
{
        RemoveWaters(Atoms);
        gridLocation2(Cubes, Atoms, cubeSize, maxhs);
}
void determineNearestForLattice(lattice &Cubes, vector<AtomStruct> &Atoms, Real maxhs)
{
        int natom=Atoms.size();
        ...
        for (int xbin=0;xbin<MaxXBin;xbin++)
        {
                for (int ybin=0;ybin<MaxYBin;ybin++)
                {
                        for (int zbin=0;zbin<MaxZBin;zbin++)
                        {
                                FindNearestHydrationRadius(Cubes[xbin][ybin][zbin], Atoms, natom, LargestRadius);
                        }
                }
        }
}
...
int CountNumCubesInProtein(lattice &Cubes)
{
        int count=0;
        int MaxXBin, MaxYBin, MaxZBin;
        ...
        cout <<"NumCubesInProtein= "<<count<<endl;
        return count;
}

void fillInMissingCubes3(lattice &Cubes, int window)
{
        int xend, yend, zend;
        int xstart, ystart, zstart;
        int MaxXBin, MaxYBin, MaxZBin;
        int numZero;
        Real nonZeroDensity=10e-10;
        Get3DVectorSize(Cubes, MaxXBin, MaxYBin, MaxZBin, "Cubes");
        cout <<"In fillInMissingCubes3"<<endl;
        for (int i=0;i<MaxXBin-1;i++)
        {
                for (int j=0;j<MaxYBin-1;j++)
                {
                        for (int k=0;k<MaxZBin-1;k++)
                        {
                                if (Cubes[i][j][k].density==0 && Cubes[i][j][k].LocatedIn==InProtein)
                                {
                                        xstart=i-window;
                                        ystart=j-window;
                                        zstart=k-window;
                                        ...
                                        numZero=0;
                                        for (int xbin=xstart;xbin<=xend;xbin++)
                                        {
                                                for (int ybin=ystart;ybin<=yend;ybin++)
                                                {
                                                        for (int zbin=zstart;zbin<=zend;zbin++)
                                                        {
                                                                if (Cubes[xbin][ybin][zbin].density==0 && (xbin==xstart || ybin==ystart || zbin==zstart || xbin==xend || ybin==yend || zbin==zend) && Cubes[xbin][ybin][zbin].LocatedIn!=InSolution && !(xbin==i && ybin==j && zbin==k)) numZero++;
                                                        }
                                                }
                                        }
                                        if (numZero==0)
                                        {
                                                Cubes[i][j][k].density=RandDouble(-1.0, 1.0);
                                                Cubes[i][j][k].density*=nonZeroDensity;
                                                Cubes[i][j][k].LocatedIn=InHydrationShell;
                                        }
                                }
                        }
        ...
        //printDesignation(Cubes);
}

void fillInMissingCubes2(lattice &Cubes)
{
        bool changed=false;
        int xend, yend, zend;
        int xstart, ystart, zstart;
        int MaxXBin, MaxYBin, MaxZBin;
        int window=2, numZero;
        Real nonZeroDensity=10e-10;
        Get3DVectorSize(Cubes, MaxXBin, MaxYBin, MaxZBin, "Cubes");
        cout <<"In fillInMissingCubes2"<<endl;
        for (int i=0;i<MaxXBin-1;i++)
        {
                for (int j=0;j<MaxYBin-1;j++)
                {
                                ...
                                if (Cubes[i][j][k].density==0 && Cubes[i][j][k].LocatedIn==InProtein)
                                {
                                        ...
                                        for (int xbin=xstart;xbin<=xend;xbin++)
                                        {
                                                for (int ybin=ystart;ybin<=yend;ybin++)
                                                {
                                                        for (int zbin=zstart;zbin<=zend;zbin++)
                                                        {
                                                                if (Cubes[xbin][ybin][zbin].density==0 && Cubes[xbin][ybin][zbin].LocatedIn!=InSolution) numZero++;
                                                        }
                                                }
                                        }
                                        if (numZero<3)
        ...
        if (changed) fillInMissingCubes2(Cubes);
        fillInMissingCubes3(Cubes, 1);
        fillInMissingCubes3(Cubes, 2);
        fillInMissingCubes3(Cubes, 3);
        fillInMissingCubes3(Cubes, 4);
        cout <<"Leaving fillInMissingCubes2"<<endl;
        //printDesignation(Cubes);
}

void fillInMissingCubes(lattice &Cubes)
{
        int MaxXBin, MaxYBin, MaxZBin;
        Real nonZeroDensity=10e-10;
        Get3DVectorSize(Cubes, MaxXBin, MaxYBin, MaxZBin, "Cubes");

        for (int i=1;i<MaxXBin-1;i++)
        {
                for (int j=1;j<MaxYBin-1;j++)
                {
                        for (int k=1;k<MaxZBin-1;k++)
                        {
                                if (Cubes[i][j][k].density==0)
                                {
                                        if (Cubes[i-1][j][k].density!=0 && Cubes[i+1][j][k].density!=0)
                                        {
                                                if (Cubes[i][j-1][k].density!=0 && Cubes[i][j+1][k].density!=0)
                                                {
                                                        if (Cubes[i][j][k-1].density!=0 && Cubes[i][j][k+1].density!=0)
                                                        {
                                                                Cubes[i][j][k].density=RandDouble(-1.0, 1.0);
                                                                Cubes[i][j][k].density*=nonZeroDensity;
                                                                Cubes[i][j][k].LocatedIn=InHydrationShell;
        ...
        //printDesignation(Cubes);
        fillInMissingCubes2(Cubes);
}

void printLocation(vector<CubeStruct> &Cubes)
{
        int ncube=Cubes.size();
        ofstream proteinFile, hydrationShellFile, solutionFile;

        OpenFileForWriting("/home2/jouko/Density/TempPdb/InProtein.pdb", proteinFile);
        OpenFileForWriting("/home2/jouko/Density/TempPdb/InHydrationShell.pdb", hydrationShellFile);
        OpenFileForWriting("/home2/jouko/Density/TempPdb/InSolution.pdb", solutionFile);
        for (int i=0;i<ncube;i++)
        {
                if (Cubes[i].LocatedIn==InProtein)
                {
                        PrintPdbLine(proteinFile, true, 0, "O", "HOH", 0, Cubes[i].x, Cubes[i].y, Cubes[i].z, 1.0, 1.0);
                }
                if (Cubes[i].LocatedIn==InHydrationShell)
                {
                        PrintPdbLine(hydrationShellFile, true, 0, "O", "HOH", 0, Cubes[i].x, Cubes[i].y, Cubes[i].z, 1.0, 1.0);
                }
                if (Cubes[i].LocatedIn==InSolution)
                {
                        PrintPdbLine(solutionFile, true, 0, "O", "HOH", 0, Cubes[i].x, Cubes[i].y, Cubes[i].z, 1.0, 1.0);
                }
        }
}
        ...
        for (xbin=0; xbin<MaxXBin; xbin++)
        {
                ...
                for (ybin=0; ybin<MaxYBin; ybin++)
                {
                        for (zbin=0; zbin<MaxZBin; zbin++)
                        {
                                ...
                                if (Cubes[xbin][ybin][zbin].LocatedIn==InProtein)
                                {
                                        //PrintPdbLine("/home2/jouko/Density/TempPdb/InProtein.pdb", true, 1, "O", "HOH", 1, Cubes[xbin][ybin][zbin].x, Cubes[xbin][ybin][zbin].y, Cubes[xbin][ybin][zbin].z, 1.0, 1.0);

                                }
                                if (abs(Cubes[xbin][ybin][zbin].x+22.6)<d && abs(Cubes[xbin][ybin][zbin].y+15.0)<d && abs(Cubes[xbin][ybin][zbin].z-2.6)<d)
                                {
                                        BoolPrint=true;
                                }
                                else BoolPrint=false;
                                if (Cubes[xbin][ybin][zbin].LocatedIn!=InProtein)
                                {
                                        ...
                                        for (xbin2=xbin; xbin2<xend; xbin2++)
                                        {
                                                ...
                                                {
                                                        XPlusBlocked=true;
                                                        if (BoolPrint) cout <<"XPlus is Blocked"<<endl;
                                                        break;
                                                }
                                        }
...
void getSurfaceFromCubes(lattice &Cubes, vector<bool> &boolSurface)
{
        int MaxXBin, MaxYBin, MaxZBin;
        ...
        for (int i=0;i<MaxXBin;i++)
        {
                for (int j=0;j<MaxYBin;j++)
                {
                        for (int k=0;k<MaxZBin;k++)
                        {
                                boolSurface[Cubes[i][j][k].nearest]=true;
                        }
                }
        }
}
...
void Cavity2(lattice &Cubes, vector<AtomStruct> &Atoms, ParamStruct &params)
{
        ...
        Real xmin, ymin, zmin;
        Real xmax, ymax, zmax;
        Real xminp, yminp, zminp;
        Real xmaxp, ymaxp, zmaxp;
        Real x, y, z;
        Real dx, dy, dz;
        ...
        SafeAlloc(boolSurface, false, natom, "boolSurface");
        MinMax(xmin, ymin, zmin, xmax, ymax, zmax, Cubes);
        MinMax(xminp, yminp, zminp, xmaxp, ymaxp, zmaxp, Atoms);
        getSurfaceFromCubes(Cubes, boolSurface);
        Print3DVectorSize(Cubes, "Cubes");
        for (int i=0;i<natom;i++)
        {
                if (BoolPrint) cout <<"i= "<<i<<endl;
                if (boolSurface[i]);
                {
                                        ...
                                        if (dist<CutOff)
                                        {
                                                ...
                                                for (d=0;d<1.0;d+=inc)
                                                {
                                                        ...
                                                        if (BoolPrint)
                                                        {
                                                                cout <<"xmin= "<<xmin<<" ymin= "<<ymin<<" zmin= "<<zmin<<endl;
                                                                cout <<"xmax= "<<xmax<<" ymax= "<<ymax<<" zmax= "<<zmax<<endl;
                                                                cout <<"xminp= "<<xminp<<" yminp= "<<yminp<<" zminp= "<<zminp<<endl;
                                                                cout <<"xmaxp= "<<xmaxp<<" ymaxp= "<<ymaxp<<" zmaxp= "<<zmaxp<<endl;
                                                                cout <<"x= "<<x<<" y= "<<y<<" z= "<<z<<endl;
                                                                cout <<"xbin= "<<xbin<<" ybin= "<<ybin<<" zbin= "<<zbin<<endl;
                                                                PrintAtomInfo(Atoms[j]);
                                                                PrintAtomInfo(Atoms[i]);
                                                                cout <<"Atom.HetAtom= "<<Atoms[j].HetAtom<<endl;
                                                                cout <<"Atom.AtomNumber= "<<Atoms[j].AtomNumber<<endl;
                                                                cout <<"Atom.AtomName= "<<Atoms[j].AtomName<<endl;
                                                                cout <<"Atom.ResidueName= "<<Atoms[j].ResidueName<<endl;
                                                                cout <<"Atom.ResidueNum= "<<Atoms[j].ResidueNum<<endl;
                                                                cout <<"Atom.x= "<<Atoms[j].x<<endl;
                                                                cout <<"Atom.y= "<<Atoms[j].y<<endl;
                                                                cout <<"Atom.z= "<<Atoms[j].z<<endl;
                                                                cout <<"Atom.weight= "<<Atoms[j].weight<<endl;
                                                        }
...
void Cavity(vector<CubeStruct> &Cubes, vector<AtomStruct> &Atoms, ParamStruct &params)
{
        ...
        if (params.ConvexConcave) Cavity(CubeLattice, params);
...
inline bool GetDensity(Real &density, CubeStruct &Cube, vector<PrdfStruct> &prdfs, Real BulkDensity, ParamStruct &params, vector<AtomStruct> &Atoms, bool BoolPrint)
{
        int SolventType, MaxIntDist, Size, NumSolventTypes;
        SolventType=Cube.AtomType;
        MaxIntDist=prdfs[ATOM_TYPES].density[SolventType].size();
        NumSolventTypes=prdfs[ATOM_TYPES].density.size();
        density=UNK_DENSITY;
        BoolPrint=false;
        ...
        Size=prdfs[ATOM_TYPES].density[SolventType].size();
        //cout <<"Size= "<<Size<<endl;
        if (Cube.IntDist<MaxIntDist)
        {
                density=prdfs[ATOM_TYPES].density[SolventType][Cube.IntDist];
        }
        ...
        if (Cube.MinDist>params.MaxDistForZeroDensity && density==0) density=UNK_DENSITY;
        ...
        if (density==UNK_DENSITY && (prdfs[ATOM_TYPES].AngularDependence || prdfs[ATOM_TYPES].AngularDependence2 || prdfs[ATOM_TYPES].PhobicPhilic || prdfs[ATOM_TYPES].SecondNearestNeighbor || prdfs[ATOM_TYPES].ConvexConcave || prdfs[ATOM_TYPES].ConvexConcave2))
        {
                SolventType=Atoms[Cube.nearest].AtomType;
                MaxIntDist=prdfs[NORMAL_ATOM_TYPES].density[SolventType].size();
                if (Cube.IntDist<MaxIntDist)
                {
                        density=prdfs[NORMAL_ATOM_TYPES].density[SolventType][Cube.IntDist];
                        if (BoolPrint) cout <<"NormalDensityMatrix= "<<prdfs[NORMAL_ATOM_TYPES].density[SolventType][Cube.IntDist]<<endl;
                }
                if (Cube.MinDist>params.MaxDistForZeroDensity && density==0) density=UNK_DENSITY;
        }
        if (density==UNK_DENSITY) 
        {
                ...
                MaxIntDist=prdfs[ELEMENTS].density[SolventType].size();
                if (Cube.IntDist<MaxIntDist)
                {
                        if (BoolPrint) cout <<"ElementsDensityMatrix= "<<prdfs[ELEMENTS].density[SolventType][Cube.IntDist]<<endl;
                        density=prdfs[ELEMENTS].density[SolventType][Cube.IntDist];
                }
                if (Cube.MinDist>params.MaxDistForZeroDensity && density==0) 
                {
                        if (BoolPrint)
                        {
                                cout <<"Set density to UNK_DENSITY"<<endl;
                        }
                        density=UNK_DENSITY;
                }
                if (BoolPrint) 
                {
                        cout <<"Cube.MinDist= "<<Cube.MinDist<<" MaxDistForZeroDensity= "<<params.MaxDistForZeroDensity<<endl;
                        cout <<"UNK_DENSITY= "<<UNK_DENSITY<<endl;
                        cout <<"Set density to "<<density<<endl;
                        if (Cube.MinDist>params.MaxDistForZeroDensity)
                        {
                                cout <<"Cube.MinDist>MaxDistForZeroDensity"<<endl;
                        }
                        if (density==0)
                        {
                                cout <<"density==0"<<endl;
        ...
        if (density==UNK_DENSITY && (prdfs[ATOM_TYPES].AngularDependence || prdfs[ATOM_TYPES].AngularDependence2 || prdfs[ATOM_TYPES].PhobicPhilic || prdfs[ATOM_TYPES].SecondNearestNeighbor || prdfs[ATOM_TYPES].ConvexConcave || prdfs[ATOM_TYPES].ConvexConcave2))
        {
                SolventType=Atoms[Cube.nearest].AtomType2;
                MaxIntDist=prdfs[NORMAL_ELEMENTS].density[SolventType].size();
                if (Cube.IntDist<MaxIntDist)
                {
                        density=prdfs[NORMAL_ELEMENTS].density[SolventType][Cube.IntDist];
                        if (BoolPrint) cout <<"NormalElementsDensityMatrix= "<<prdfs[NORMAL_ELEMENTS].density[SolventType][Cube.IntDist]<<endl;
                        if (Cube.MinDist>params.MaxDistForZeroDensity && density==0) density=UNK_DENSITY;
                }
        }
...
void VerySafeMakeGrid(Real xmin, Real ymin, Real zmin, Real xmax, Real ymax, Real zmax, vector<AtomStruct> &Atoms, Real maxhs, Real CubeSize, vector<CubeStruct> &Cubes, string PdbFile, vector<PrdfStruct> &prdfs, Real BulkDensity, ParamStruct &params)
{
                                ...
                                if (true)
                                {
                                        ...
                                        if (LocatedIn==InHydrationShell) 
                                        {
                                                ...
                                                if (Cube.MinDist<maxhs)
                                                {
                                                        ...
                                                        GetDensity(Cube.density, Cube, prdfs, BulkDensity, params, Atoms, false);
...
void AssignCubeDensities(vector<CubeStruct> &Cubes, vector<AtomStruct> &Atoms, vector<PrdfStruct> &prdfs, Real BulkDensity, bool UniformHydrationShellDensity, Real HydrationShellDensity, ParamStruct &params)
{
        ...
        BoolPrint=IsNotEmpty(prdfs[ATOM_TYPES].density);
        ...
        for (n=0;n<GridPoints;n++)
        {
                ...
                if (Cubes[n].LocatedIn==InHydrationShell)
                {
                         ...
                         */
                        Cubes[n].IntDist=int(Cubes[n].MinDist/prdfs[0].RecBin+0.5);
                        start=clock();
                        //if (n>13400) cout <<"About to enter GetDensity"<<endl;
                        SetDensity=GetDensity(density, Cubes[n], prdfs, BulkDensity, params, Atoms, BoolPrint);
                        ...
                        if (!SetDensity && BoolPrint)
                        {
                                ...
                                PrintCubeInfo(Cubes[n]);
                                cout <<"prdfs[0].density["<<Atoms[Cubes[n].nearest].AtomType<<"]["<<Cubes[n].IntDist<<"]= "<<prdfs[0].density[Atoms[Cubes[n].nearest].AtomType][Cubes[n].IntDist]<<endl;
                                cout <<endl;
                                NumSetToBulk++;
                        }
...
void AssignCubeDensities(vector<CubeStruct> &Cubes, vector<AtomStruct> &Atoms, vector<PrdfStruct> &prdfs, ParamStruct &params)
{
        bool SetDensity=true, BoolPrint=true;
        int n, GridPoints=Cubes.size();
        int NumSetToBulk=0;
...
bool Angular(vector<AtomStruct> &Atoms, CubeStruct &Cube, ParamStruct &params, int index)
{
        ...
        /*
           if (Atoms[nearest].atomid==2)
           {
           for (int i=0;i<int(Atoms[nearest].connectivity.size());i++)
           {
           bonded=Atoms[nearest].connectivity[i];
           if (BoolPrint) 
           {
...
void SetCubeTypes(vector<CubeStruct> &Cubes, vector<AtomStruct> &Atoms, ParamStruct &params)
{
        int ncube=Cubes.size();
        vector<int> Hydrophobic;
        if (params.ConvexConcave || params.ConvexConcave2) Cavity(Cubes, Atoms, params);
...
void RoundGrid(Real &xmin, Real &ymin, Real &zmin, Real &xmax, Real &ymax, Real &zmax, Real cubeSize)
{
        xmin=Round(xmin-cubeSize*5.0, cubeSize);
        ymin=Round(ymin-cubeSize*5.0, cubeSize);
        zmin=Round(zmin-cubeSize*5.0, cubeSize);
        xmax=Round(xmax+cubeSize*5.0, cubeSize);
        ymax=Round(ymax+cubeSize*5.0, cubeSize);
        zmax=Round(zmax+cubeSize*5.0, cubeSize);
}
...
void SetUpGrid(vector<CubeStruct> &Cubes, vector<AtomStruct> &Atoms, ParamStruct &params)
{
        ...
        if (params.RoundGrid) RoundGrid(xmin, ymin, zmin, xmax, ymax, zmax, params.CubeSize);
...
void BuildHydrationShell(vector<CubeStruct> &Cubes, vector<AtomStruct> &Atoms, vector<PrdfStruct> &prdfs, Real BulkDensity, bool UniformHydrationShell, Real hsdensity, ParamStruct &params)
{
        SetUpGrid(Cubes, Atoms, params);
        AssignCubeDensities(Cubes, Atoms, prdfs, BulkDensity, UniformHydrationShell, hsdensity, params);
        //if (!params.EliminateCavities) EliminateCavities(xmin, ymin, zmin, 0, Cubes, CubeSize);
}

void BuildHydrationShell(vector<CubeStruct> &Cubes, vector<AtomStruct> &Atoms, vector<PrdfStruct> &prdfs, ParamStruct &params)
{
        SetUpGrid(Cubes, Atoms, params);
        AssignCubeDensities(Cubes, Atoms, prdfs, params);
}
