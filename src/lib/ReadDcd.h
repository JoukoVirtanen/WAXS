	...
	if (file.is_open())
	{
		...
		for (int i=0;i<20;i++)
		{
			file.read(reinterpret_cast<char *>(&cont), sizeof(int));
                        SafePushBack(control, cont, "control");
		}
		...
		for (int i=0;i<1;i++)
		{
			file.read(reinterpret_cast<char *>(&Double), 8);
			if (verbose) cout <<"Int= "<<Int<<endl;
		}
		if (verbose) cout <<"file.tellg()= "<<file.tellg()<<endl;
		position=file.tellg();
        ...
        {
                cout <<"ERROR: Unable to open DCD file "<<DCDFile;
                exit(EXIT_FAILURE);
        }
}
#endif
